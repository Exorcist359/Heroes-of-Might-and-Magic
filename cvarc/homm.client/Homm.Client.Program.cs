...
namespace Homm.Client
{
    class Program
    {
        ...
        //My method
        public Dictionary<UnitType, int> isCellSafe(Location cell, HommSensorData sensorData)
        {
            var objOnWay = sensorData.Map.Objects
                                .Where(obj => obj.Location.X == cell.X && obj.Location.Y == cell.Y)
                                .FirstOrDefault();

            var myArmy = sensorData.MyArmy;
            //Dictionary<UnitType, int> otherArmy = new Dictionary<UnitType, int>();
            return Combat.Resolve(new ArmiesPair(myArmy, otherArmy)).IsAttackerWin;
        }
